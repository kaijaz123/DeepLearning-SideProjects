...
class MLP:
    def __init__(self,input_neurons,hidden_neurons,output,bias=0.5):
        self.input_neurons = input_neurons + 1 # <- add one more neuron for bias
        self.hidden_neurons = [hidden_neurons[0] + 1] # <- add one more neuron for bias
        self.output = output
        self.bias = bias
        ...
        for index in range(len(self.networks)):
            activations = np.zeros(self.networks[index] - 1)
            if index != len(self.networks)-1:
                activation_bias = np.zeros(1)
                activation = np.append(activations, activation_bias, axis = 0)
    ...
    def forward_pass(self,input):
        bias = np.ones(1)
        activations = np.append(input,bias)
    ...
    def backpropogation(self,error):
        ...
        for i in reversed(range(len(self.networks) - 1)):
            ...
            # print(self.sigmoid_derivative(activation))
    ...
    def train(self,inputs,targets,epochs,learning_rate):
        for i in range(epochs):
            ...
            print('Errors: {} at epoch {}'.format(sum_errors/(j+1),i+1))
